...
public class AccountAddInputActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setContentView(R.layout.activity_account_addinput);
		...
		childIdx = typeDB.getTypesByParentIdx(parentIdx).get(0).getIdx();
		findViews();
	}
	...
	public void btnClick(View view) {
		...
		case R.id.typeLayout:
			//选择类别
			intent = new Intent(AccountAddInputActivity.this, TypeListActivity.class);
		...
		case R.id.findbackwayLayout:
			intent = new Intent(AccountAddInputActivity.this, TextInputActivity.class);
		...
		case R.id.add:
			if(getAndValidateViewDatas()) {
				...
				if(childIdx == 0 || childIdx == 1) {
					imgIdx = R.drawable.qq;
				}else {
					imgIdx = typeDB.getImgIdxByIdx(childIdx);
				}
	...
	private void saveAccount() {
		if(accountDB.exitsByName(nameStr, myApp.getUsername())) {
			Toast.makeText(this, "账户标题重复，请修改!", 0).show();
			nameEt.requestFocus();
		}else {
			accountDB.saveAccount(myApp.getUsername(), account);
			Toast.makeText(this, "新建账户成功!", 0).show();
			back();
		}
	}
